    ...
    <>
      ...
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-[80] transition-all duration-300 pointer-events-auto w-[calc(100vw-32px)] sm:w-auto flex justify-center">
        {isMinimized ? (
          ...
          >
            <span className="font-serif text-vermilion">YUJI</span>
            <span>CURATOR</span>
          ...
          <div className="inline-flex flex-nowrap justify-center items-center gap-1.5 px-3 py-2 bg-ink/95 border border-gold/50 rounded-2xl shadow-2xl backdrop-blur-md text-xs font-mono text-paper">
            ...
            <div className="hidden sm:flex items-center gap-2 pr-2 border-r border-ink-border shrink-0">
              <span className="font-serif text-sm font-bold text-vermilion">YUJI</span>
...
function AnnouncementLiveEditModal({
  ...
  const [arabicForm, setArabicForm] = useState<AnnouncementArabicConfig>(
    initialArabicConfig || { text: "استخدم كود YUJI10 للحصول على خصم 10% عند أول طلب لك" }
  );
...
function EditorialLiveEditModal({
  ...
  const [arabicForm, setArabicForm] = useState<EditorialArabicConfig>(
    initialArabicConfig || {
      ...
      authenticityGuaranteeText: "نضمن أصالة 100% لجميع المجلدات والروايات المعروضة في YUJI. طبعات يابانية رسمية ومرخصة بدون أي نسخ مقلدة.",
      ...
      footerDescription: "دار YUJI — الأرشيف التحريري الأول في مصر المتخصص في استيراد وتوفير أندر مجلدات المانجا والروايات الخفيفة وبوكس سيت المقتنين الأصلية بأعلى معايير الجودة.",

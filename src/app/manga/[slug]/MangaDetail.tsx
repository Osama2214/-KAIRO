...
export default function MangaDetailView({ volume: initialVolume }: { volume: MangaVolume }) {
  ...
  // Keep the route's full text when the index omits descriptions.
  const volume = React.useMemo(() => productWithDetails(initialVolume, liveVolume), [initialVolume, liveVolume]);
  ...
  const activeVolumes = storeVolumes;
